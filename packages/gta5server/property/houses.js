...
try
{
    ...
    houses.refresh = id =>
    {
        ...
        const labelText = `
            ${enums.housesType[houses.getType(id)]} #${houses.getID(id)}\n
            ${enums.housesClass[houses.getClass(id)]} класса\n\
            Владелец: ${houses.getOwner(id).id ? "~b~" : ""}${houses.getOwner(id).name}\n
            ~w~Стоимость: ${func.formatCash(container.get('houses', id, 'price'))}${houses.getLocked(id) ? "\n\n~r~Закрыт" : ""}`
        ...
        if(container.get('houses', id, 'interior').x !== 0
            ...
            && container.get('houses', id, 'interior').z !== 0)
        {
            container.set('houses', id, '_markerExit', mp.markers.new(1, new mp.Vector3(container.get('houses', id, 'interior').x, container.get('houses', id, 'interior').y, container.get('houses', id, 'interior').z - 1.0), 1, {
                    color: [ 255, 255, 255, 100 ],
                    dimension: houses.getID(id) + 50
                }))
            container.set('houses', id, '_colshapeExit', mp.colshapes.newCircle(container.get('houses', id, 'interior').x, container.get('houses', id, 'interior').y, 1.0, houses.getID(id) + 50)).setVariable('houseID', id)
        }

        if(houses.getOwner(id).id <= 0
            && houses.getType(id) === 0
            && container.get('houses', id, 'dimension') === 0)
        {
        ...
        if(container.get('houses', id, 'garage').position.x !== 0
            ...
            && container.get('houses', id, 'garage').position.z !== 0)
        {
            container.set('houses', id, '_markerHouseGarage', mp.markers.new(1, new mp.Vector3(container.get('houses', id, 'garage').house.x, container.get('houses', id, 'garage').house.y, container.get('houses', id, 'garage').house.z - 1.0), 1, {
                    color: [ 255, 255, 255, 100 ],
                    dimension: houses.getID(id) + 50
                }))
            container.set('houses', id, '_colshapeHouseGarage', mp.colshapes.newCircle(container.get('houses', id, 'garage').house.x, container.get('houses', id, 'garage').house.y, 1.0, houses.getID(id) + 50)).setVariable('houseID', id)

            container.set('houses', id, '_colshapeGarage', mp.colshapes.newCircle(container.get('houses', id, 'garage').position.x, container.get('houses', id, 'garage').position.y, 4.0, container.get('houses', id, 'dimension'))).setVariable('houseID', id)
            container.set('houses', id, '_markerGarage', mp.markers.new(36, new mp.Vector3(container.get('houses', id, 'garage').position.x, container.get('houses', id, 'garage').position.y, container.get('houses', id, 'garage').position.z), 2.0, {
                    color: [ 255, 255, 255, 100 ],
                    dimension: container.get('houses', id, 'dimension')
                }))
            container.set('houses', id, '_labelGarage', mp.labels.new(`Гараж дома #${houses.getID(id)}${houses.getLocked(id) ? "\n\n~r~Закрыт" : ""}`, new mp.Vector3(container.get('houses', id, 'garage').position.x, container.get('houses', id, 'garage').position.y, container.get('houses', id, 'garage').position.z + 1), {
                    font: 0,
                    drawDistance: 4,
                    dimension: container.get('houses', id, 'dimension')
                }))

            container.set('houses', id, '_colshapeGarageExit', mp.colshapes.newCircle(container.get('houses', id, 'garage').vehicle.x, container.get('houses', id, 'garage').vehicle.y, 4.0, houses.getID(id) + 50)).setVariable('houseID', id)

            container.set('houses', id, '_markerCharacterGarage', mp.markers.new(1, new mp.Vector3(container.get('houses', id, 'garage').character.x, container.get('houses', id, 'garage').character.y, container.get('houses', id, 'garage').character.z - 1.0), 1, {
                    color: [ 255, 255, 255, 100 ],
                    dimension: houses.getID(id) + 50
                }))
            container.set('houses', id, '_colshapeCharacterGarage', mp.colshapes.newCircle(container.get('houses', id, 'garage').character.x, container.get('houses', id, 'garage').character.y, 1.0, houses.getID(id) + 50)).setVariable('houseID', id)
        }
    }
    ...
    houses.tpInterior = (player, id, admin = false) =>
    {
        ...
        if(houses.getLocked(id)
            && !admin)return user.notify(player, 'Двери закрыты', 'error')
    ...
    houses.action = player =>
    {
        ...
        if(garage)
        {
            ...
            else houses.tpGarage(player, id)
            return
        }

        if(!interior) houses.tpInterior(player, id)
        else houses.tp(player, id)
    }
