const hud =
{
    speedometer:
    {
        ...
        on: () =>
        {
            hud.speedometer.speed(0)
            $('#hud .speedometer').addClass('speedometer-on')

            let count = 0
            let timer = setInterval(() =>
            {
                hud.speedometer.speed(count)
                count ++
        ...
        locked: status =>
        {
            if(!status) $('#hud .speedometer #lock').removeClass('on')
            else $('#hud .speedometer #lock').addClass('on')
        },
        fuel: value =>
        {
            $('#hud .speedometer .fuel h1').html(`${value.toFixed(1)}`)
        }
    },
    cash:
    {
...
$(document).ready(() =>
{
    rage.addEvent('UI::hud', data =>
    {
        switch(data.cmd)
        {
            ...
            case 'speedometerToggle':
            {
                hud.speedometer.toggle(data.data)
                break
            }
            case 'speedometerOn':
            {
            ...
            case 'speedometerSpeed':
            {
                hud.speedometer.speed(data.data)
                break
            }
            case 'speedometerMileage':
            {
                hud.speedometer.mileage(data.data)
                break
            }
            case 'speedometerLocked':
            {
                hud.speedometer.locked(data.data)
                break
            }
            case 'speedometerFuel':
            {
                hud.speedometer.fuel(parseFloat(data.data))
                break
            }
        }
    })
